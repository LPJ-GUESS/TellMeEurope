...
input <- yaml::yaml.load_file(file.path("C:\\Users\\Admin\\Documents\\tellus\\TellMeEurope_Set_Ups\\European_applications\\TellMeEurope.yml"))
...
if (is_LC <- as.logical(input[["Switches"]][["is_LC"]])){
  LC <- read.table(input[["Directory"]][["Landcover"]], header = T)
}
